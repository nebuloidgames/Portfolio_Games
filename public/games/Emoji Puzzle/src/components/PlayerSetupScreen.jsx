...
import logoBlackVertical from '../assets/nebuloid-vertical.png';
...
export default function PlayerSetupScreen({
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-between p-3 sm:p-5 md:p-6 select-none overflow-hidden font-sans">
      ...
      <div className="relative w-full max-w-4xl flex items-center justify-center z-20 pt-1 pb-2">
        <div className="flex items-center gap-2.5 px-4 py-1.5 rounded-2xl bg-white/90 backdrop-blur-md border border-white/60 shadow-lg transition-transform hover:scale-105">
          ...
          <div className="hidden sm:flex flex-col text-left leading-tight pr-1">
            <span className="text-[10px] font-black tracking-widest text-black uppercase">NEBULOID TECH STUDIO LLP</span>
          </div>
        </div>
      </div>
      ...
      {showKeyboard && (
        <div
          className={`fixed z-50 w-[94%] max-w-[430px] bg-slate-950/95 backdrop-blur-2xl border-2 border-blue-400/40 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.85)] p-2 sm:p-2.5 text-white select-none transition-shadow ${
            isDragging ? 'shadow-[0_25px_60px_rgba(59,130,246,0.6)] ring-2 ring-blue-400' : ''
          }`}
          style={{
            left: '50%',
            bottom: '20px',
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="w-5 h-5 rounded-full bg-white/10 hover:bg-white/20 text-white font-bold text-[11px] flex items-center justify-center cursor-pointer transition-colors"
                title="Hide Keyboard"
              >
                ✕
              </button>
          ...
          <div className="grid grid-cols-10 gap-1 mb-1">
            {numberRow.map((num) => (
              <button
                ...
                className="h-7 sm:h-8 rounded-md bg-white/10 hover:bg-white/25 active:bg-blue-600 active:scale-95 text-white font-black text-[11px] sm:text-xs transition-all flex items-center justify-center border border-white/10 cursor-pointer shadow-xs"
              >
                {num}
              </button>
            ))}
          ...
          {/* Row 3: A - L */}
          <div className="grid grid-cols-9 gap-1 mb-1 px-2 sm:px-3">
            {row2.map((char) => (
              <button
                key={char}
                type="button"
          ...
          <div className="flex gap-1 mb-1">
            ...
            <button
              ...
              className={`w-11 sm:w-13 h-7 sm:h-8 rounded-md font-black text-[10px] transition-all flex items-center justify-center border cursor-pointer active:scale-95 ${
                isCaps
            ...
            <button
              ...
              className="w-11 sm:w-13 h-7 sm:h-8 rounded-md bg-rose-600/80 hover:bg-rose-500 text-white font-black text-xs transition-all flex items-center justify-center border border-rose-400/40 cursor-pointer active:scale-95 shadow-sm"
              title="Backspace"
            >
              ⌫
            </button>
          ...
          <div className="flex gap-1 pt-0.5">
            ...
            <button
              type="button"
              onClick={handleSpace}
              className="flex-1 h-7 sm:h-8 rounded-md bg-white/10 hover:bg-white/20 active:bg-white/30 text-slate-300 font-bold text-[10px] sm:text-[11px] uppercase tracking-widest transition-all flex items-center justify-center border border-white/10 cursor-pointer active:scale-98 shadow-xs"
            >
              SPACE
            </button>
      ...
      {!showKeyboard && (
        <div className="relative z-10 pt-2 pb-1 text-center">
          <span className="text-[11px] font-bold text-white/90 tracking-wider uppercase drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
            Nebuloid Tech Studio LLP • Emoji Puzzle Player Profile
          </span>
        </div>
      )}
    </div>
